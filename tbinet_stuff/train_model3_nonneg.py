...
output = Conv1D(320,kernel_size=5,padding="same",activation="relu")(sequence_input)
...
#BiLSTM Layer
output = Bidirectional(LSTM(320,return_sequences=True,kernel_constraint=nonneg(),recurrent_constraint=nonneg()))(output)
...
#FC_output = Dense(10,kernel_constraint=nonneg())(FC_output)
#FC_output = Activation('relu')(FC_output)
...
checkpointer = ModelCheckpoint(filepath="./model3_nonneg/tbinet.{epoch:02d}-{val_loss:.2f}.hdf5", verbose=1, save_best_only=False)
...
model.save('./model3_nonneg/tbinet.h5')
####### Learning curve
...
plt.savefig("./model3_nonneg/model_loss.png")
